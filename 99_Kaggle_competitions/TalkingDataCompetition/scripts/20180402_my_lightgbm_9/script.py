...
# max_depth  :  4  ->  5
# subsample  : 0.7 -> 0.9
# train set : no start date up to 2017-11-08 23:59:59
# validation set : end date 2017-11-09 23:59:59
# romove 4 features: minute, second, ip_min_dev, ip_sec
# no device
# max_rounds : 1000 -> 1500
...
def data_log( df ):
    ...
    print("Log of features inside hour...")
    df['ip_hr_log'] = np.log(1 + df['ip_hr'])
    df.drop( ['ip_hr'], axis=1, inplace=True )
    gc.collect()
    df['ip_hr_app_log'] = np.log(1 + df['ip_hr_app'])
    df.drop( ['ip_hr_app'], axis=1, inplace=True )
    gc.collect()
    df['ip_hr_os_log'] = np.log(1 + df['ip_hr_os'])
    df.drop( ['ip_hr_os'], axis=1, inplace=True )
    gc.collect()
    df['ip_hr_dev_log'] = np.log(1 + df['ip_hr_dev'])
    df.drop( ['ip_hr_dev'], axis=1, inplace=True )
    gc.collect()
    
    print("Log of features inside minute...")
    df['ip_min_log'] = np.log(1 + df['ip_min'])
    df.drop( ['ip_min'], axis=1, inplace=True )
    gc.collect()
    df['ip_min_app_log'] = np.log(1 + df['ip_min_app'])
    df.drop( ['ip_min_app'], axis=1, inplace=True )
    gc.collect()
    df['ip_min_os_log'] = np.log(1 + df['ip_min_os'])
    df.drop( ['ip_min_os'], axis=1, inplace=True )
    gc.collect()
    df['ip_min_dev_log'] = np.log(1 + df['ip_min_dev'])
    df.drop( ['ip_min_dev'], axis=1, inplace=True )
    gc.collect()
    
    print("Log of features inside second...")
    df['ip_sec_log'] = np.log(1 + df['ip_sec'])
    df.drop( ['ip_sec'], axis=1, inplace=True )
    gc.collect()
