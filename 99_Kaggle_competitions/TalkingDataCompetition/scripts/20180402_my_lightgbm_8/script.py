...
def data_normalization( df ):
    ...
    print("Normalization of features inside hour...")
    df['ip_hr'] = (df['ip_hr'] - train_ip_hr_mean)/train_ip_hr_std 
    gc.collect()
    df['ip_hr_app'] = (df['ip_hr_app'] - train_ip_hr_app_mean)/train_ip_hr_app_std
    gc.collect()
    df['ip_hr_os'] = (df['ip_hr_os'] - train_ip_hr_os_mean)/train_ip_hr_os_std
    gc.collect()
    df['ip_hr_dev'] = (df['ip_hr_dev'] - train_ip_hr_dev_mean)/train_ip_hr_dev_std
    gc.collect()
    
    print("Normalization of features inside minute...")
    df['ip_min'] = (df['ip_min'] - train_ip_min_mean)/train_ip_min_std
    gc.collect()
    df['ip_min_app'] = (df['ip_min_app'] - train_ip_min_app_mean)/train_ip_min_app_std
    gc.collect()
    df['ip_min_os'] = (df['ip_min_os'] - train_ip_min_os_mean)/train_ip_min_os_std
    gc.collect()
    df['ip_min_dev'] = (df['ip_min_dev'] - train_ip_min_dev_mean)/train_ip_min_dev_std
    gc.collect()
    
    print("Normalization of features inside second...")
...
gc.collect()
val_df = data_normalization( val_df )
gc.collect()
...
gc.collect()
test_df = data_normalization( test_df )
gc.collect()
