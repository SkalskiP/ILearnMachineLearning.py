...
def prep_data( df ):
    ...
    print('group by : app_hh')
    gp = df[['app', 'hour', 'day', 'channel']].groupby(by=['app', 'day',
             'hour'])[['channel']].count().reset_index().rename(index=str, 
             columns={'channel': 'app_hh'})
    df = df.merge(gp, on=['app','day','hour'], how='left')
    del gp
    print( "app_hh max value = ", df.nip_hh_dev.max() )
    df['app_hh'] = df['app_hh'].astype('uint32')
    gc.collect()
    print( df.info() )

    df.drop( ['ip','day', 'device'], axis=1, inplace=True )
    gc.collect()
    print( df.info() )
...
predictors = ['app','os', 'channel', 'hour', 'nip_day_test_hh', 'nip_day_hh',
              'nip_hh_os', 'nip_hh_app', 'nip_hh_dev', 'app_hh']
